...
export default function ResizableSplit({ left, right, rightColRef, className = "" }) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`flex items-center justify-center w-[20px] h-[30px] border border-1 bg-white shadow-sm transition-colors mb-10 duration-150 rounded-[2px] ${
        ...
        >
          <ArrowLeftRight
            className={`h-[12px] w-[12px] transition-colors duration-150 ${
              isHovering || isDragging ? "text-[#1a5fb4] stroke-[2.5]" : "text-gray-800 stroke-[2]"
            }`}
          />
        </div>
      </div>
