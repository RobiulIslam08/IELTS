...
import { sections, totalQuestions } from "./readingData";
 ...
 const handleFinalSubmit = async () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setIsSaving(true);
        try {
          // localStorage থেকে ইউজার ডাটা এবং টেস্ট টাইপ নেওয়া হচ্ছে
          const userData = JSON.parse(localStorage.getItem('user'));
          const userId = userData?.id;
          const test_type = userData?.test_type || 'Academic'; // fallback হিসেবে Academic রাখা হলো
          ...
          // payload-এ test_type যুক্ত করা হলো
          const payload = {
            user_id: userId,
            module_type: 'reading', 
            test_type: test_type, // <--- এই ফিল্ডটি ডাটাবেজে ইনসার্ট হওয়ার জন্য অত্যন্ত জরুরি
            exam_id: Number(examId),
            test_id: Number(testNumber),
            answers: formattedAnswers,
          };

          // আপনার কন্ট্রোলারের মেথড অনুযায়ী সঠিক এন্ডপয়েন্ট কল করা হচ্ছে
  ...
  const getSectionForQuestion = (qNum) => {
    if (qNum >= 1 && qNum <= 14) return 1;
    if (qNum >= 15 && qNum <= 27) return 2;
    return 3;
  };
  ...
  return (
    <main className="h-screen w-screen flex flex-col bg-stone-50 text-[16px] text-black overflow-hidden select-none">
        ...
        <div className="mx-auto flex w-full items-center justify-between gap-6 max-[820px]:flex-wrap max-[820px]:gap-y-2">
          <div className="flex items-center gap-6 max-[820px]:w-full max-[820px]:justify-between">
            ...
            <div className="grid gap-0.5 content-center justify-items-start text-[13px] text-stone-700">
              ...
              <span className="text-stone-500 font-medium">IELTS Reading General</span>
            </div>
          </div>
      ...
      {/* (নোট: টপ পার্ট নেভিগেশন এখান থেকে সম্পূর্ণ রিমুভ করা হয়েছে কারণ এটি এখন ফুটারে থাকবে) */}

      {/* 2. MAIN WORKSPACE AREA (FULL COMPUTER SCREEN HEIGHT) */}
      ...
      <footer className="flex-shrink-0 min-h-[95px] w-full border-t border-stone-300 bg-[#f4f4f4] flex flex-col justify-center px-6 py-2 z-20 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] gap-2">
            ...
            {sections.map((sec) => (
              <button
                ...
                onClick={() => {
                  ...
                  // পার্ট চেঞ্জ হলে ঐ পার্টের প্রথম কোশ্চেনে অটো ফোকাস সেট করতে পারেন
        ...
        {/* LAYER 2: ডাইনামিক কোশ্চেন গ্রিড (শুধুমাত্র সিলেক্টেড পার্টের নাম্বার দেখাবে) */}
